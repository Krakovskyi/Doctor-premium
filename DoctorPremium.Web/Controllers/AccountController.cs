...
namespace DoctorPremium.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager,
            ...
            ILanguageService languageService, ITimeZoneService timeZoneService, ISessionDataService sessionDataService)
        {
        ...
        private void CreateSessionData()
        {
            ...
            {
                if (HttpContext.Session["SessionDataId"] == null)
                {
                    string sessionId = this.Session.SessionID;
                    string ipaddress = HttpContext.Request.UserHostAddress;
                    int newItemId = _sessionDataService.CreateNewSession(sessionId, DateTime.UtcNow, ipaddress, UserInfoHelper.User.AspNetUserId);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, string CaptchaText,  bool checkboxLicanse)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PhoneNumber=code };
        ...
        public async Task<ActionResult> ConfirmEmail(int userId, string code)
        {
            ...
            bool success = false;
            if (code.Length < 10) //new code
            {
                var user = UserManager.FindById(userId);
                if (user != null)
                {
                    success = user.PhoneNumber != null ? user.PhoneNumber == code : false;
                    if (success)
                    {
            ...
            return View(success ? "ConfirmEmail" : "Error");
        }
		...
		public async Task<ActionResult> ConfirmEmailManual(ConfirmEmailManualViewModel model)
		{
            ...
            //SetTokenProvider();
            //var result = await UserManager.ConfirmEmailAsync(model.UserId, HttpUtility.UrlDecode(model.ConfirmCode));

            var code = model.ConfirmCode.Trim();
            bool success = false;
            if (code.Length < 10) //new code
            {
            ...
            {
                SetTokenProvider();
                var result = await UserManager.ConfirmEmailAsync(model.UserId, HttpUtility.UrlDecode(code));
                success = result.Succeeded;
                if (!success)
                {
